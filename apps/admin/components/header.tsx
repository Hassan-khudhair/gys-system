...
import { User, Sun, Moon, Globe, X, Dumbbell, LogOut, AlertTriangle } from "lucide-react";
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          {(title || subtitle) && (
            <div>
              {title    && <h1 className="text-sm font-semibold text-text truncate max-w-37.5 md:max-w-none">{title}</h1>}
              {subtitle && <p className="text-[10px] text-muted truncate max-w-37.5 md:max-w-none">{subtitle}</p>}
            </div>
          )}
        </div>
              ...
              <div className="absolute inset-e-0 top-full mt-2 w-72 max-w-[calc(100vw-2rem)] bg-surface border border-border/60 rounded-xl shadow-2xl shadow-black/30 z-50 overflow-hidden">
                ...
                <div className="px-4 pb-4 pt-1">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-red-500/10 border border-red-500/25 text-red-500 hover:bg-red-500/20 hover:border-red-500/40 active:scale-[0.98] transition-all font-semibold text-[14px]"
                  >
      ...
      {showConfirm && (
        ...
        >
          <div className="w-full max-w-sm bg-surface border border-border/60 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden">
            {/* Icon area */}
            <div className="flex flex-col items-center px-6 pt-7 pb-5">
              <div className="w-14 h-14 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-4">
                <AlertTriangle className="w-7 h-7 text-red-500" />
              </div>
              <h2 className="text-[17px] font-bold text-text text-center mb-2">{t("confirm_signout")}</h2>
              <p className="text-[13px] text-muted text-center leading-relaxed">{t("confirm_signout_msg")}</p>
            </div>
            ...
            <div className="px-4 pb-5 space-y-2.5">
              <button
                ...
                disabled={signingOut}
                className="w-full py-3.5 rounded-xl bg-red-500 hover:bg-red-600 active:scale-[0.98] text-white font-bold text-[15px] transition-all disabled:opacity-60 flex items-center justify-center gap-2.5"
              >
                {signingOut ? (
                  <>
              ...
              <button
                ...
                disabled={signingOut}
                className="w-full py-3 rounded-xl bg-surface-2/60 hover:bg-surface-2 border border-border/40 text-text font-medium text-[14px] transition-all disabled:opacity-60"
              >
                {t("cancel")}
              </button>
            </div>
