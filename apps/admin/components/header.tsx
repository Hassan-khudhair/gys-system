...
export function Header({ title, subtitle, gymName, adminName, email, onMenuClick }: HeaderProps) {
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={dialogRef}>
          ...
          >
            ...
            <span className="hidden sm:inline text-sm text-muted">{adminName ?? t("gym_admin_role")}</span>
          </button>
          ...
          {showInfo && (
            <div className="absolute inset-e-0 top-full mt-2 w-72 bg-surface border border-border rounded-xl shadow-2xl z-50 overflow-hidden">
              ...
              <div className="px-4 py-3 space-y-3">
                {gymName && (
                  <div>
                    <p className="text-xs text-muted mb-0.5">{t("gym_name_label")}</p>
                    <p className="text-sm font-medium text-text">{gymName}</p>
                  </div>
                )}
                {adminName && (
                  <div>
                ...
                <div>
                  <p className="text-xs text-muted mb-0.5">{t("gym_admin_role")}</p>
